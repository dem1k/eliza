...
class Auth extends Controller {
    ...
    function index()
    {
        ...
        elseif (!$this->ion_auth->is_admin())
        {
            ...
            redirect($this->config->item('admin'), 'refresh');
        }
        else
        {
 ...
 public function register()
{
    ...
    $this->data['title'] = "Регистрация пользователя";
    ...
    $this->form_validation->set_rules('phone1', 'Телефон', 'required|xss_clean|min_length[5]|max_length[13]');
    $this->form_validation->set_rules('company', 'Город', 'required|xss_clean');
    ...
    $is_valid=false;
    if ($is_valid = $this->form_validation->run() == true)
    {
    ...
    if ($is_valid &&$this->ion_auth->register($username, $password, $email, $additional_data,'client'))
...
//            var_dump($this->data['identity']);die;
            $this->data['identity_human'] = $identity_human;
            $this->data['template'] = 'auth/forgot_password';
            $this->load->view('client/main', $this->data);
        }
        else
        {
